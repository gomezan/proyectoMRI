...
from DCSRNyunzeman.tf_dcsrn import util
from DCSRNyunzeman.tf_dcsrn.layers import (weight_variable, conv3d, pixel_wise_softmax_2)
from DCSRNyunzeman.ssim import tf_SSIM
...
def create_conv_net(x, channels=1, layers=4, growth_rate=24, filter_size=3, summaries=True):
    ...
    for layer in range(0, layers):
        ...
        bn = tf.compat.v1.layers.batch_normalization(in_node, 0)
        # ELU layer
        elu = tf.nn.elu(bn)
        # 3D conv layer
        conv1 = conv3d(elu, w1)
...
class DCSRN(object):
    ...
    def __init__(self, channels=1, **kwargs):
        ...
        self.x = tf.compat.v1.placeholder("float", shape=[None, None, None, None, channels])
        self.y = tf.compat.v1.placeholder("float", shape=[None, None, None, None, channels])
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=3500, epochs=10, display_step=10, restore=False, write_graph=False, prediction_path = 'prediction'):
        ...
        with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            summary_writer = tf.compat.v1.summary.FileWriter(output_path, graph=sess.graph)
